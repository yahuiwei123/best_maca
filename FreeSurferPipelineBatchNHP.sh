...
for Subject in `echo $Subjlist | sed -e 's/@/ /g'` ; do
  ...
  T1wImage="${StudyFolder}/${Subject}/T1w/T1w_acpc_dc_restore.nii" #T1w FreeSurfer Input (Full Resolution)
  T1wImageBrain="${StudyFolder}/${Subject}/T1w/T1w_acpc_dc_restore_brain.nii" #T1w FreeSurfer Input (Full Resolution)
  T2wImage="${StudyFolder}/${Subject}/T1w/T2w_acpc_dc_restore.nii" #T2w FreeSurfer Input (Full Resolution)
  FSLinearTransform="${HCPPIPEDIR_Templates}/fs_xfms/eye.xfm" #Identity
  T2wFlag="${T2wFlag:=T2w}" # T2w, FLAIR or NONE. Default is T2w
  SPECIES="Macaque"
  # the next line is uncommended by XL, I think we need GCAdir for other than defort human file
  GCAdir="${HCPPIPEDIR_Templates}/MacaqueYerkes19" #Template Dir with FreeSurfer NHP GCA and TIF files
  # XL, I cannot find this file, and I also think it's not nessary to do the rescale
  ...
  # ccpons="130 118 133 130 194 96"  # added by XL, should change it accordin to different monkeys, voxel coords of the seed for the corpus callosum   voxel coords of the seed for the pons
  ...
  ############################################### Modified until here by Takuya Hayashi Nov 4th 2015.
     ...
     ${HCPPIPEDIR}/FreeSurfer/FreeSurferPipelineNHP.sh \
      ...
      --seed="$Seed" \
      --printcom="$PRINTCOM" \
      --firstpass="$FirstPass"     # added by XL
